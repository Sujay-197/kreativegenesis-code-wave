  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/chat/simple`, {
          method: 'POST',
  ...
  const handleConfirmGenerate = useCallback(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Backend returns an app_id; for now we still use local HTML generation
        // since the backend stores HTML server-side. We can fetch it later via /api/apps/{id}.
